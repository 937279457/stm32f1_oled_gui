...
MYI2C_InitTypeDef* MYI2C_InitStruct;

void mpu6050_DMP_I2C_Init(MYI2C_InitTypeDef* pMYI2C_InitStruct)
{
    MYI2C_InitStruct = pMYI2C_InitStruct;
}

uint8_t I2C_Write(uint8_t DeviceAddress, uint8_t RegisterAddress, uint32_t Times, uint8_t* pData)
{
    MYI2C_Write(MYI2C_InitStruct, DeviceAddress, RegisterAddress, pData, Times);
    return 0;
}

uint8_t I2C_Read(uint8_t DeviceAddress, uint8_t RegisterAddress, uint32_t Times, uint8_t* pData)
{
    MYI2C_Read(MYI2C_InitStruct, DeviceAddress, RegisterAddress, pData, Times);
    return 0;
}

#define i2c_write(deaddr, readdr, size, pdata)   I2C_Write((deaddr<<1), readdr, size, pdata)
#define i2c_read(deaddr, readdr, size, pdata)   I2C_Read((deaddr<<1), readdr, size, pdata)
#define delay_ms(ms)    Delay_ms(ms)
#define get_ms(ms)      Get_ms1(ms)
void Get_ms1(unsigned long* ms){}
